...
const ProfileDetails = ({ activeTab, id, profileData, isOwner, fetchProfile }) => {
  ...
  const handleLike = async (postId) => {
    const updatedLikes = new Set(likePosts);
    ...
    setLikePosts(updatedLikes);
    localStorage.setItem("likePosts", JSON.stringify(Array.from(updatedLikes)));

    try {
      await handleLikePost(postId);
      await fetchUserPost(id); // ✅ FIXED
    } catch (error) {
      console.error(error);
  ...
  const tabContent = {
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="space-y-2 mb-4 dark:text-gray-300">
                <div className="flex items-center"><Home className="mr-2" />{profileData?.bio?.liveIn}</div>
                <div className="flex items-center"><Heart className="mr-2" />{profileData?.bio?.relationship}</div>
                <div className="flex items-center"><MapPin className="mr-2" />{profileData?.bio?.hometown}</div>
                <div className="flex items-center"><Briefcase className="mr-2" />{profileData?.bio?.workplace}</div>
                <div className="flex items-center"><GraduationCap className="mr-2" />{profileData?.bio?.education}</div>
              </div>
    ...
    about: (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
        <Card>
          <CardContent className="p-6">
            <h2 className="font-bold mb-4">About {profileData?.username}</h2>
            <div className="space-y-4">
              ...
              <div className="flex items-center"><Heart className="mr-2" />{profileData?.bio?.relationship}</div>
              <div className="flex items-center"><MapPin className="mr-2" />{profileData?.bio?.hometown}</div>
              <div className="flex items-center"><Phone className="mr-2" />{profileData?.bio?.phone}</div>
              <div className="flex items-center"><Mail className="mr-2" />{profileData?.email}</div>
              <div className="flex items-center"><Cake className="mr-2" />Birthday: {formatDateInDDMMYYY(profileData?.dateOfBirth)}</div>
            </div>
          </CardContent>
        </Card>
      </motion.div>
